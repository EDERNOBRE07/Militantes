...
export const UnifiedNeighborhoodAudit: React.FC<UnifiedNeighborhoodAuditProps> = ({
  ...
  // Número de fotos por página na galeria (pelo menos 15 fotos por página, conforme solicitado)
  ...
  // Lista unificada de todas as fotos de comprovação deste bairro
  const allNeighborhoodPhotos = useMemo(() => {
    ...
    enrichedCheckIns.forEach(item => {
      item.photos.forEach(p => {
        list.push({
          photo: p,
          streetName: item.streetName,
          timestamp: item.timestamp,
          chkId: item.checkIn.id,
          pinNum: item.pinNum,
          militantName: item.militantName,
  ...
  const totalMateriais = enrichedCheckIns.reduce((acc, c) => {
    const m = c.materials;
    return acc + (m.santinhos || 0) + (m.adesivo_bola || 0) + (m.adesivo_parachoque || 0) + (m.colinhas || 0);
  }, 0);

  // Paginação da galeria única de fotos (mínimo 15 por página)
  ...
  return (
    <div className="space-y-6 pt-3" id={`unified-audit-${bairro.id}`}>
      ...
      {/* 1. TABELA ÚNICA DE RUAS DO BAIRRO (COM COLUNA MILITANTE) */}
              ...
              {enrichedCheckIns.length === 0 ? (
                ...
                enrichedCheckIns.map(item => (
                  ...
                  >
                    ...
                    {/* COLUNA MILITANTE (SOLICITADA EXPLICITAMENTE PELO USUÁRIO) */}
                    ...
                    <td className="py-2.5 px-3 text-center whitespace-nowrap">
                      {item.photos.length > 0 ? (
                        <button
                          onClick={() => onZoomPhoto(item.photos[0])}
            ...
            {enrichedCheckIns.length > 0 && (
              <tfoot>
                <tr className="bg-slate-100/90 font-bold text-slate-800 border-t-2 border-slate-300 text-xs">
                  ...
                  <td className="py-2.5 px-3 text-center text-emerald-700 font-mono">{allNeighborhoodPhotos.length} fotos</td>
      ...
      {/* 2. GALERIA DE FOTOS ÚNICA DO BAIRRO (PELO MENOS 15 FOTOS POR PÁGINA) */}
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-2xs space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Total de <strong>{totalPhotos} fotos comprovatórias</strong> anexadas aos lançamentos de ruas deste bairro
            </p>
          </div>
